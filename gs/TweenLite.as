			...
			if (this.target is Array) {
				...
				for (i = 0; i < endArray.length; i++) {
					if (this.target[i] != endArray[i] && this.target[i] != undefined) {
						this.tweens[this.tweens.length] = {o:this.target, p:i.toString(), s:this.target[i], c:endArray[i] - this.target[i]}; //o: object, p:property, s:starting value, c:change in value,
					}
				}
			} else {
				...
				if (v.frame != null && _isDisplayObject) {
					addSubTween(frameProxy, {frame:this.target.currentFrame}, {frame:v.frame}, {target:this.target});
				}
				...
				for (p in v) {
					if (p == "ease" || p == "delay" || p == "overwrite" || p == "onComplete" || p == "onCompleteParams" || p == "runBackwards" || p == "visible" || p == "persist" || p == "onUpdate" || p == "onUpdateParams" || p == "autoAlpha" || p == "onStart" || p == "onStartParams" ||p == "renderOnStart" || p == "proxiedEase" || p == "easeParams" || ($hrp && $reservedProps.indexOf(" " + p + " ") != -1)) { 
						...
						//if (this.target.hasOwnProperty(p)) { //REMOVED because there's a bug in Flash Player 10 (Beta) that incorrectly reports that DisplayObjects don't have a "z" property. This check wasn't entirely necessary anyway - it just prevented runtime errors if/when developers tried tweening properties that didn't exist.
							if (typeof(v[p]) == "number") {
								this.tweens[this.tweens.length] = {o:this.target, p:p, s:this.target[p], c:v[p] - this.target[p]}; //o:object, p:property, s:starting value, c:change in value
							} else {
								this.tweens[this.tweens.length] = {o:this.target, p:p, s:this.target[p], c:Number(v[p])}; //o:object, p:property, s:starting value, c:change in value
							}
						//}
					}
				}
		...
		protected function addSubTween($proxy:Function, $target:Object, $props:Object, $info:Object = null):void {
			...
			for (var p:String in $props) {
				if (typeof($props[p]) == "number") {
					this.tweens[this.tweens.length] = {o:$target, p:p, s:$target[p], c:$props[p] - $target[p], sub:sub}; //o:Object, p:Property, s:Starting value, c:Change in value, sub:Subtween object;
				} else {
					this.tweens[this.tweens.length] = {o:$target, p:p, s:$target[p], c:Number($props[p]), sub:sub};
				}
			}
