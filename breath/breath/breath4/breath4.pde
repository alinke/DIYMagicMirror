...
const int readyLEDPin = 6;       // analog 3 pin
const int buttonPin = 5;          // the pin that the pushbutton is attached to, triggers breathalyzer
const int blowLEDPin = 14;
const int waitLEDPin = 19;

const int level0LEDPin = 16;
const int level1LEDPin = 17;
...
void setup() {
  ...
  Serial.print("initial switch value= ");
  ...
  Serial.println(initialPot);
   
  range = map(initialPot, 0, 1024, 0, 3);
  Serial.print("initial pot range is: ");
...
void loop() {
  ...
  //Serial.print("baseline and baselineDifference= ");
  ...
  if (initialButtonState != currentButtonState && ready == 1 && playingFlag == 0) { //if button switched and alcohol sensor ready and not already playing
      playingFlag = 1;
      buttonPushCounter++;
